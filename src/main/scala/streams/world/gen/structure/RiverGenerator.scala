...
class RiverGenerator(val liquid: MaterialLiquid, dimensionId: Int) extends StructureGenerator[RiverStructure](-1, dimensionId) {
    ...
    override protected def generate(world: WorldServer, xChunk: Int, zChunk: Int, primer: ChunkPrimer) {
        ...
        if(!structures.contains(riverKey)) {
            ...
            implicit val random = chunkRandom(xRiverChunk, zRiverChunk)(world)
        ...
        structures.get(riverKey).foreach(_.foreach{ river =>
            val blockArray = new ChunkBlockArrayAccess(world, xChunk, zChunk, primer)
            river.carveValleys(blockArray, chunkRandom(xChunk, zChunk)(world)) // 1st pass, before chunk terrain replacement
        })
    }
