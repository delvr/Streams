...
class BlockRiverIce(val dx: Int, val dz: Int) extends BlockIce with FixedFlowBlock {
    ...
    override def removedByPlayer(world: World, pos: BlockPos, player: EntityPlayer, willHarvest: Boolean) = changeToWater(pos)(world)

    override def harvestBlock(w: World, player: EntityPlayer, pos: BlockPos, state: IBlockState, tileEntity: TileEntity) {}
